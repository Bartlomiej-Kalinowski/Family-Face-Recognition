...
class FaceDatabase:
    ...
    def save_face(self, face_img, face_id, original_path, bbox, embedding=None, is_test=0):
        ...
        try:
            ...
            # self._conn.commit() # Dobra praktyka: commituj w kontrolerze po paczce zdjęć
        except Exception as e:
            print(f"[BŁĄD SQL] save_face: {e}")

    def set_manual_label(self, face_id: str, label: str, is_test: int = 0) -> None:
        """Zapisuje etykietę użytkownika """
        self._cursor.execute('''
    ...
    def get_unlabeled_test_data(self) -> list:
        """
        Pobiera dane testowe, które nie mają jeszcze etykiety manualnej.
        Zwraca listę krotek: (face_id, image_path, embedding_np)
        """
        self._cursor.execute('''
        ...
        # Konwersja embeddingów z JSON na numpy array
        processed_data = []
        for fid, path, emb_json, bbox in rows:
            try:
                ...
                print(f"[BŁĄD] Nie udało się sparsować embeddingu dla {fid}: {e}")
    ...
    def mark_unlabeled_as_test(self):
        """Oznacza wszystkie niepodpisane twarze jako zbiór testowy."""
        self._cursor.execute('UPDATE faces SET is_test = 1 WHERE manual_label IS NULL')
        self._conn.commit()

    def get_label_by_id(self, face_id) -> str:
        """
        Pobiera aktualną etykietę dla konkretnego face_id.
        Priorytet: manual_label > svm_label.
        """
        query = 'SELECT COALESCE(manual_label, svm_prediction) FROM faces WHERE face_id = ?'

        # Przekazujemy face_id jako krotkę (face_id,) - wymóg sqlite3 dla parametrów
        self._cursor.execute(query, (face_id,))

        result = self._cursor.fetchone()

        # fetchone() zwraca krotkę (np. ("Aneta",)) lub None.
        # Musimy wyciągnąć pierwszy element [0].
        return result[0] if result and result[0] else "Nieznany"